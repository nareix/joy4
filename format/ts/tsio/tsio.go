...
func (self *PMT) Unmarshal(b []byte) (n int, err error) {
	...
	for n < len(b) {
		...
		info.StreamType = b[n]
		if info.StreamType == 0x0f {
			info.StreamTypeDes = "aac"
		} else if info.StreamType == 0x1b {
			info.StreamTypeDes = "h264"
		} else {
			info.StreamTypeDes = "other"
		}
		n++
...
func ParsePESHeader(h []byte) (hdrlen int, streamid uint8, datalen int, pts, dts time.Duration, err error) {
	...
	streamid = h[3]
	// spew.Printf("streamid %x", streamid)
